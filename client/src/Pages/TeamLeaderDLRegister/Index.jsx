      ...
      <form method='POST' onSubmit={handleOnSubmit} style={{ overflow: 'hidden' }} >
        <div className='outer'>
          <div className='top'>
            ...
            <input type="text" name="empID" id="empID" placeholder='Enter Emplyee Id' style={Styles.topText} onChange={handleInputChange} />
          </div>
          <div className='bottom'>
            <div className='bottomLeft'>
              <label htmlFor="name">Name  </label>
              <input type="text" name="name" id="name" placeholder='Enter Employee Name' onChange={handleInputChange} />

              <label htmlFor="mobile">Mobile </label>
              <input type="number" name='mobile' id='mobile' placeholder='Enter Number' onChange={handleInputChange} />
                ...
                <section style={{ overflowY: 'auto', height: '150px', width: '95%', margin: 'auto', display: 'flex', flexWrap: 'wrap', justifyContent: 'flex-start', alignItems: 'flex-start', gap: '10px' }}>
                  ...
                  {showStateList !== null && showStateList.map((state, index) => (
                    <div key={state._id} style={{ display: 'flex', width: 'min-content', gap: '4.5px' }}>
                      <input type="checkbox" name="state" id={state._id} value={state._id} style={{ width: 'min-content' }} onChange={(event)=>handleChangeCheckBoxes(event,index)} checked ={currStateStatus[index]?true:false}/>
