...
const Index = () => {
    ...
    useEffect(() => {
        if(isAuthenticated.employeeId !== null){
            console.log(isAuthenticated.employeeId);
        }
    ...
    const callStageCountNnumberOfTimes = async(count, showAllStages) => {
        ...
        for(; index < count; index++){
            ...
            const leadsValue = await stageCountForDashboardApi('', stageId, '', '', employeeId);
            const TODAYS = await stageCountForDashboardApi('', stageId, today.toDateString(), today.toDateString(), employeeId);
            const LAST_WEEK = await stageCountForDashboardApi('', stageId, lastSevenDays, today.toDateString(), employeeId);
            const LAST_MONTH = await stageCountForDashboardApi('', stageId, lastMonth, today.toDateString(), employeeId);
