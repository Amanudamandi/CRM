...
const LeadBoardDl = ({ data }) => {
    ...
    const headingList =
        [
            'Stage', 'Name', 'Mobile', 'Email', 'Type', 'State', 'District', 'Source', 'Date', 'Intersted IN', 'Proposal ', 'Remark'
        ];
    ...
    // console.log("my data is  clicked  open update: ", selectedLead)
            ...
            <section style={{ ...Styles.employeeContainer, display: (leadClickedInfo.clicked) ? 'none' : 'block' }}>
                <table style={Styles.employeeTable} rules='rows' onScroll={(event) => {
                    ...
                    <ShowClickableDealerLeads
                        ...
                        employeeAutoGeneratedID={isAuthenticated.employeeId}
                        coordinatorAutoGeneratedID=''
                        setClicked={setLeadClickedInfo}
                        boolLeadHandler={false}
                        // showCheckBoxes={true}
                        limit={limit}
                        storeFilterData={storeFilterData}
