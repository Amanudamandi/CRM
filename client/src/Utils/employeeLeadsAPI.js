...
export const employeeLeads = async( setIndividualLeads, employeeAutoGeneratedID = '', coordinatorAutoGeneratedID = '', pageCount=1, limit = 15, startDate='', endDate='', name='', email='', mobile='', source='' , stage='', state='',  district='', kwpInterested='', type='', employeeName, teamLeaderName,  setIsLoading = null ) => {
    if(setIsLoading !== null ) setIsLoading(true);
    try{
        const sendRequest = await axios.get(`${process.env.REACT_APP_URL}/client/fetchClients/?page=${pageCount}&limit=${limit}&empID=${employeeAutoGeneratedID}&TLID=${coordinatorAutoGeneratedID}&startDate=${startDate}&endDate=${endDate}&name=${name}&email=${email}&mobile=${mobile}&source=${source}&kwpInterested=${kwpInterested}&type=${type}&stage=${stage}&state=${state}&district=${district}&employeeName=${employeeName === undefined ? '' : employeeName}&teamLeaderName=${teamLeaderName === undefined ? '' : teamLeaderName}`);
